...
async def test_superadmin_can_delete_department_with_users(test_client, admin_headers):
    ...
    try:
        ...
        department_admin = next(
            (user for user in users_before_delete if user["user_id"] == department_payload["admin_user_id"]), None)
        ...
        migrated_admin = next(
            (user for user in users_after_delete if user["id"] == department_admin_id), None)
        assert migrated_admin is not None
        assert migrated_admin["department_name"] == "Default Department"

        migrated_user = next(
            (user for user in users_after_delete if user["id"] == created_user_id), None)
        assert migrated_user is not None
        assert migrated_user["department_name"] == "Default Department"
    finally:
...
async def test_superadmin_cannot_delete_default_department(test_client, admin_headers):
    ...
    default_department = next(
        (department for department in departments if department["name"] == "Default Department"), None)
