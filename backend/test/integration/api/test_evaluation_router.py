...
async def _upload_test_benchmark(test_client, admin_headers: dict[str, str], db_id: str) -> tuple[str, str]:
    ...
    line = '{"query":"What is unit testing?","gold_answer":"Automated tests used to verify code behavior"}\n'

    response = await test_client.post(
        f"/api/evaluation/databases/{db_id}/benchmarks/upload",
        data={"name": benchmark_name,
              "description": "pytest benchmark for download"},
        files={"file": ("pytest_benchmark.jsonl", line.encode(
