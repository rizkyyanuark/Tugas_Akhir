...
def test_list_subagents_returns_data(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.subagent_router.service.get_all_subagents", fake_get_all_subagents)
...
def test_get_single_subagent(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.subagent_router.service.get_subagent", fake_get_subagent)
...
def test_create_subagent(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.subagent_router.service.create_subagent", fake_create_subagent)
...
def test_create_subagent_duplicate_returns_409(monkeypatch):
    async def fake_create_subagent(data, created_by, db=None):
        raise IntegrityError(
            ...
            {},
            Exception(
                'duplicate key value violates unique constraint "subagents_pkey"'),
        )
...
def test_update_subagent(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.subagent_router.service.update_subagent", fake_update_subagent)
...
def test_delete_subagent(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.subagent_router.service.delete_subagent", fake_delete_subagent)
...
def test_update_subagent_status(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.subagent_router.service.set_subagent_enabled", fake_set_subagent_enabled)

    app = _build_app()
    client = TestClient(app)
    resp = client.put(
        "/api/system/subagents/research-agent/status", json={"enabled": False})
    assert resp.status_code == 200, resp.text
    payload = resp.json()
    assert payload["success"] is True
    assert payload["data"]["enabled"] is False
    assert captured == {"name": "research-agent",
                        "enabled": False, "updated_by": "admin"}
...
def test_update_subagent_status_not_found(monkeypatch):
    ...
    app = _build_app()
    client = TestClient(app)
    resp = client.put("/api/system/subagents/missing/status",
                      json={"enabled": True})
    assert resp.status_code == 404, resp.text
