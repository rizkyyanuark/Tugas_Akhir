...
def _build_app(*, allow_superadmin: bool) -> FastAPI:
    ...
    async def fake_superadmin_user():
        if not allow_superadmin:
            raise HTTPException(
                status_code=403, detail="Superadmin privileges are required")
        return User(
            username="root",
            user_id="root",
            password_hash="x",
...
def test_list_skills_route_returns_data(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.skill_router.list_skills", fake_list_skills)
...
def test_import_skill_route_accepts_skill_md(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.skill_router.import_skill_zip", fake_import_skill_zip)
...
def test_update_skill_file_passes_operator(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.skill_router.update_skill_file", fake_update_skill_file)
...
def test_dependency_options_route(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.skill_router.get_skill_dependency_options", fake_get_skill_dependency_options)
...
def test_update_skill_dependencies_route(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.skill_router.update_skill_dependencies", fake_update_skill_dependencies)
...
def test_list_remote_skills_route(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.skill_router.list_remote_skills", fake_list_remote_skills)

    app = _build_app(allow_superadmin=True)
    client = TestClient(app)
    resp = client.post("/api/system/skills/remote/list",
                       json={"source": "anthropics/skills"})
    assert resp.status_code == 200, resp.text
    payload = resp.json()
    assert payload["success"] is True
    assert payload["data"] == [
        {"name": "frontend-design", "description": "demo"}]
...
def test_install_remote_skill_route(monkeypatch):
    ...
    monkeypatch.setattr(
        "server.routers.skill_router.install_remote_skill", fake_install_remote_skill)
