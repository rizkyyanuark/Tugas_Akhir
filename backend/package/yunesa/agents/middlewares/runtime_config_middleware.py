...
from yunesa.agents import load_chat_model
...
class RuntimeConfigMiddleware(AgentMiddleware):
    """Runtime configuration middleware for model/tool/MCP/prompt overrides.

    Knowledge-base tools are moved to KnowledgeBaseMiddleware.
    Skills features are moved to SkillsMiddleware.

    Supports custom context field names so different scenarios
    (e.g. main agent / sub-agent) can use different config fields.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            extra_tools: Extra tool list (passed from create_agent tools argument).
            model_context_name: Context field name for model (default: "model").
            system_prompt_context_name: Context field name for system prompt (default: "system_prompt").
            tools_context_name: Context field name for tools list (default: "tools").
            knowledges_context_name: Context field name for knowledge list (default: "knowledges").
            mcps_context_name: Context field name for MCP server list (default: "mcps").
            enable_model_override: Whether to allow model override (default: True).
            enable_system_prompt_override: Whether to allow system prompt override (default: True).
            enable_tools_override: Whether to allow tool override (default: True).
        """
        super().__init__()
        ...
        self.tools: list[Any] = []
        # Preload tool list (only when tool override is enabled).
        # Note: KB tools are moved to KnowledgeBaseMiddleware.
        if self.enable_tools_override:
            self.base_tools = get_all_tool_instances()
            self.tools = self.base_tools + (extra_tools or [])
        elif extra_tools:
            logger.warning(
                "RuntimeConfigMiddleware: extra_tools is provided, but enable_tools_override=False; "
                "extra_tools will be ignored and no tool override will be applied."
            )
    ...
    async def awrap_model_call(
        ...
        # 2. Tool override (optional).
        # Note: skill-dependent tool loading is moved to SkillsMiddleware.
        if self.enable_tools_override:
            # Resolve tools from context configuration.
    ...
    async def get_tools_from_context(self, context) -> list:
        """Get tool list from context configuration."""
        selected_tools = []
        selected_tool_names: set[str] = set()

        # 1. Base tools (filtered from context.tools).
        ...
        # 2. MCP tools (use unified entry and auto-filter disabled_tools).
