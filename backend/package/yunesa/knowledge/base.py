...
from yunesa.knowledge.utils import sanitize_processing_params
from yunesa.utils import logger
from yunesa.utils.datetime_utils import coerce_any_to_utc_datetime, utc_isoformat
...
class KnowledgeBase(ABC):
    ...
    def __init__(self, work_dir: str):
        ...
        self._metadata_loaded = False  # Flag to track if metadata is loaded

        # Initialize class-level lock
        if KnowledgeBase._processing_lock is None:
            KnowledgeBase._processing_lock = threading.Lock()

        os.makedirs(work_dir, exist_ok=True)

        # Note: Metadata is not loaded in __init__, managed by KnowledgeBaseManager
    ...
    async def create_database(
        ...
        **kwargs,
        ...
        Args:
            ...
            embed_info: Embed model information
            llm_info: LLM configuration information
            **kwargs: Other configuration parameters

        Returns:
            Database information dictionary
        """
        from yunesa.utils import hashstr

        kwargs = ensure_chunk_defaults_in_additional_params(kwargs)

        # Get is_private config from kwargs
    ...
    async def delete_database(self, db_id: str) -> dict:
        ...
        if db_id in self.databases_meta:
            ...
            # 2. Parallel deletion of all files under this db_id in knowledge base buckets
            prefix = f"{db_id}/"
            cleanup_buckets = {
    ...
    async def update_content(self, db_id: str, file_ids: list[str], params: dict | None = None) -> list[dict]:
        """
        Update content - re-parse files and update vector store based on file_ids
    ...
    def get_query_params_config(self, db_id: str, **kwargs) -> dict:
        """
        Get query parameter configurations of the knowledge base type

        Args:
            db_id: Database ID
            **kwargs: Extra parameters (like reranker_names, etc.)
    ...
    def query(self, query_text: str, db_id: str, **kwargs) -> list[dict]:
        """
        Synchronous query knowledge base (compatibility method)
    ...
    def get_database_info(self, db_id: str, include_files: bool = True) -> dict | None:
        ...
        Args:
            db_id: Database ID
            include_files: Whether to include file info, default is True (backward compatibility)

        Returns:
            Database info or None
        """
        ...
        # Check and fix anomalous processing status
        self._check_and_fix_processing_status(db_id)

        # Count number of files (always calculate even if not loading file details)
        ...
        # Only load file details when needed
        if include_files:
            db_files = {}
            ...
            # Sort file list by creation time in descending order
            sorted_files = dict(
                sorted(
                    db_files.items(),
    ...
    def get_databases(self, include_files: bool = False) -> dict:
        ...
        Args:
            include_files: Whether to include file info, default False to reduce response size

        Returns:
            List of databases
        """
        # Ensure metadata is loaded (lazy loading mechanism)
    ...
    def _is_file_in_processing_queue(cls, file_id: str) -> bool:
        ...
        Returns:
            bool: Whether the file is in processing queue
        """
        with cls._processing_lock:
    ...
    def _check_and_fix_processing_status(self, db_id: str) -> None:
        """
        Check and fix anomalous processing status
        If file status is processing but not actually in the processing queue, change to corresponding error status
        ...
        try:
            ...
            # Define intermediate states to check and their corresponding error states
            intermediate_states = {
    ...
    async def get_file_content(self, db_id: str, file_id: str) -> dict:
        ...
        Returns:
            dict: Dictionary containing file content information
        """
        pass
    ...
    async def get_file_info(self, db_id: str, file_id: str) -> dict:
        """
        Get complete file information (basic info + content info) - keep backward compatibility
        ...
        Returns:
            dict: Dictionary containing file info and chunks
        """
        pass
    ...
    def update_database(self, db_id: str, name: str, description: str, llm_info: dict = None) -> dict:
        ...
        # If llm_info is provided, update (only for LightRAG type)
