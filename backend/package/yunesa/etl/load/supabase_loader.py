...
from yunesa.etl.clients.supabase_auth import create_etl_supabase_client
...
class SupabaseLoader:
    ...
    def upsert_papers(self, df: pd.DataFrame, chunk_size: int = 100) -> int:
        ...
        from yunesa.etl.transform.enricher import normalize_document_type, normalize_venue_name
        ...
        for _, row in df.iterrows():
            ...
            record = {
                ...
                "year": year,
                "journal": self._clean_value(normalize_venue_name(row.get('Journal') or row.get('journal'))),
