...
from knowledge.etl.scraping.config import (
    SAVE_DIR, PRODI_WEB_CONFIG, TARGET_PRODI_NAMES,
    SINTA_DEPTS, HEADERS, ENABLE_SCIVAL, ID_COLUMN_TYPES,
    SCIVAL_EMAIL, SCIVAL_PASS,
)
from knowledge.etl.scraping.clients.pddikti_client import PddiktiClient
from knowledge.etl.scraping.clients.simcv_client import SimCVClient
from knowledge.etl.scraping.clients.sinta_client import SintaCrawler
from knowledge.etl.scraping.clients.scholar_client import ScholarVerificationClient
from knowledge.etl.scraping.clients.scival_client import SciValClient
from knowledge.etl.scraping.core.parsers import PARSER_MAP
from knowledge.etl.scraping.utils import (
...
def run_web_step():
    ...
    from knowledge.etl.scraping.core.web_scraper import WebProdiScraper
...
def run_pddikti_step():
    ...
    print(f"\n📡 STARTING PDDIKTI COLLECTION for {len(ACTIVE_CONFIGS)} Active Configs...")
...
def run_smart_merge():
    ...
    print(f"   📊 Web Base Records: {len(web_data)}")
        
    # B. ENRICH with PDDIKTI (strict match)
...
def _run_scholar(df, sample_size=None):
    """
    Verify/Search Google Scholar IDs via Bright Data.
    
    Args:
        df: DataFrame to process
        sample_size: If set, only process this many records (for testing/saving API credits)
    """
    print("\n   [4d] Scholar Verification & Search (Bright Data)...")
    ...
    if not client.proxies:
        print("      ⚠️ Skipping Scholar Step (No Proxy Configured)")
        return df
    ...
    for idx in indices:
        ...
        # 1. VERIFY existing ID
        verified = False
        if sid and sid.lower() != 'nan' and len(sid) > 5:
            res = client.verify_id(sid, name)
        ...
        # 2. SEARCH for new ID
        if not verified and (not sid or sid.lower() == 'nan'):
            cands = client.search_google(name)
            for c in cands:
                res = client.verify_id(c, name)
                if res and res['valid']:
                    df.at[idx, 'scholar_id'] = c
                    count_new += 1
                    break
    
    print(f"      Verified: {count_valid}, Found New: {count_new}")
    return df
...
def run_enrichment(scholar_sample=None):
    ...
    """
    print("\n" + "=" * 60)
    print("📦 STEP 4: ENRICHMENT PIPELINE")
    print("=" * 60)
...
def run_supabase_sync():
    ...
    """
    from knowledge.etl.scraping.clients.supabase_client import SupabaseClient
    
    print("\n🚀 STEP 6: SUPABASE SYNCHRONIZATION")
    print("=" * 50)
    ...
    for col in id_cols:
        if col in df.columns:
            ...
            if has_dot:
                print(f"   ⚠️ {col} still has .0 suffix! Data quality issue.")
