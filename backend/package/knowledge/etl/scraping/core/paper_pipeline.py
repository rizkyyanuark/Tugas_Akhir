...
from knowledge.etl.scraping.core.config import SAVE_DIR, SCIVAL_EMAIL, SCIVAL_PASS, SERPAPI_KEY
from knowledge.etl.scraping.core.logger import logger
...
def run_scopus_scraping(email=None, password=None):
    ...
    from knowledge.etl.scraping.core.scopus_client import ScopusPaperClient

    logger.info("STEP 1: SCOPUS SCRAPING")

    # 1. Load Target IDs
    if not DOSEN_CSV.exists():
        logger.error(f"'{DOSEN_CSV}' not found!")
        return
...
def run_scopus_processing(input_raw=None, output_master=None):
    ...
    """
    from knowledge.etl.scraping.core.scopus_client import process_scopus_data

    logger.info("STEP 2: SCOPUS PROCESSING")
    ...
    if not raw_file.exists():
        logger.error(f"Raw file not found: {raw_file}. Run Step 1 first.")
        return
    ...
    logger.info(f"Saved {len(df_final)} clean & enriched papers to {master_file}")
    
    # Optional: Delete raw file after successful processing to prevent re-merging
    try:
        raw_file.unlink()
        logger.info(f"Cleaned up raw file: {raw_file.name}")
    except:
        pass
...
def run_supabase_insert():
    ...
    logger.info("STEP 3: SUPABASE INSERT")
    ...
    logger.info("Loading Cleaned Papers...")
    ...
    try:
        ...
        # 1. Upsert Papers
        logger.info("Upserting Papers to Supabase...")
        client.upsert_papers(df)

        # 2. Link Papers to Lecturers
        logger.info("Linking Papers to Lecturers...")
        client.link_papers_to_lecturers(df)

        logger.info("Insertion & Linking Complete!")
            ...
            if resp.status_code == 200:
                ...
                if articles or attempt == max_retries:
                    if attempt > 0 and articles:
                        logger.info(f"      Retry {attempt} successful ({len(articles)} articles)")
                    return articles, has_next

                # Empty result on non-last attempt → retry
                logger.warning(f"      SerpAPI returned empty (attempt {attempt+1}), retrying in 3s...")
                time.sleep(3)
            else:
                error = resp.json().get("error", resp.text[:200])
                logger.warning(f"      SerpAPI HTTP {resp.status_code}: {error}")
...
def run_scholar_scraping(api_key=None, limit_per_author=500, test_target_id=None):
    ...
    logger.info("STEP 4: GOOGLE SCHOLAR SCRAPING (3-Phase: Scrape → Dedup → Match)")
    ...
    if test_target_id:
        ...
        logger.info(f"Targeting {len(targets)} lecturers with Scholar ID.")

    # ==================================================================
    # PHASE 1: PURE SCRAPE — Fetch all papers, no filtering
    # ==================================================================
    logger.info("PHASE 1: PURE SCRAPE (No Filter, Auto-Save)")
    ...
    if SCHOLAR_TEMP_CSV.exists() and not test_target_id:
        try:
            ...
            logger.info(f"Already scraped: {len(scraped_ids)} lecturers.")
        except Exception:
            pass
    ...
    for i, t in enumerate(targets):
        ...
        if t['id'] in scraped_ids:
            logger.info(f"[{i+1}/{len(targets)}] {t['name']} ({t['id']}) ... Already scraped (Resume)")
            continue

        logger.info(f"[{i+1}/{len(targets)}] Scraping {t['name']} ({t['id']})...")
        start = 0
        author_count = 0
        ...
        if not test_target_id:
            ...
            logger.info(f"[Auto-Save] {len(all_raw_papers)} papers saved to temp.")
    ...
    if not test_target_id and newly_scraped > 0:
        pd.DataFrame(all_raw_papers).to_csv(SCHOLAR_TEMP_CSV, index=False)
        logger.info(f"Phase 1 Complete: {len(all_raw_papers)} total raw papers ({total_api_calls} API calls)")

    if not all_raw_papers:
        logger.warning("No papers found during scraping.")
        return
    ...
    # ==================================================================
    logger.info("PHASE 2: BATCH DEDUP (Scopus + Cross-Dosen)")
    ...
    if SCOPUS_CSV.exists():
        try:
            ...
            logger.info(f"Loaded {len(scopus_titles_norm)} Scopus titles for dedup.")
        except Exception:
            pass
    ...
    logger.info(f"Scopus exact dedup: {scopus_dup_count} papers removed.")
    ...
    logger.info(f"Cross-dosen exact dedup: {cross_exact_dup} papers removed.")
    ...
    logger.info(f"Fuzzy dedup on {len(df_raw)} remaining papers...")
    ...
    for idx, row in df_raw.iterrows():
        ...
        if idx > 0 and idx % 1000 == 0:
            logger.info(f"      ... processed {idx}/{len(df_raw)} ({len(fuzzy_dup_indices)} fuzzy dups, {comparisons:,} comparisons)")

    df_raw = df_raw.drop(index=fuzzy_dup_indices).reset_index(drop=True)
    logger.info(f"Fuzzy dedup: {len(fuzzy_dup_indices)} papers removed.")

    total_after = len(df_raw)
    total_removed = total_before - total_after
    logger.info(f"Dedup Summary: {total_before} → {total_after} ({total_removed} duplicates removed)")
    ...
    # ==================================================================
    logger.info("PHASE 3: BATCH AUTHOR MATCH")
    ...
    logger.info(f"Matched {matched_count} papers with multiple dosen co-authors.")
    ...
    # ==================================================================
    if test_target_id:
        logger.info("TEST MODE OUTPUT (NOT SAVING)")
        logger.info("\n" + str(df_final[['Authors', 'Author IDs', 'Title']].head(15)))
        return df_final

    df_final.to_csv(SCHOLAR_CSV, index=False)
    logger.info(f"Saved {len(df_final)} unique Scholar papers -> {SCHOLAR_CSV}")

    # Cleanup temp file after successful save
    if SCHOLAR_TEMP_CSV.exists():
        try:
            SCHOLAR_TEMP_CSV.unlink()
            logger.info(f"Cleaned up temp file: {SCHOLAR_TEMP_CSV.name}")
        except Exception:
            pass
...
def run_scholar_enrichment(input_csv=None, output_csv=None, test_limit=None):
    ...
    if dosen_csv_path.exists():
        try:
            ...
            logger.warning(f"Could not load dosen CSV: {e}")

    logger.info("SCHOLAR ENRICHMENT PIPELINE")
    logger.info("Flow: Semantic Scholar (FREE) -> OpenAlex (FREE) -> BrightData (PAID)")

    if not input_file.exists():
        logger.error(f"'{input_file}' not found! Run Scholar Scraping first.")
        return
    ...
    logger.info(f"Already enriched   : {already_done}")
    logger.info(f"Remaining          : {remaining}")
    if test_limit:
        logger.info(f"TEST MODE          : Only processing {test_limit} papers")
    ...
    for i, row in df.iterrows():
        ...
        if test_limit and processed_count >= test_limit:
            logger.info(f"TEST LIMIT reached ({test_limit} papers). Stopping.")
            break
        ...
        logger.info(f"[{processed_count}/{test_limit or remaining}] {title[:65]}")
        logger.debug(f"Status: Abstract={'Y' if abstract else 'N'} Keywords={'Y' if keywords else 'N'} DOI={'Y' if doi else 'N'}")
        ...
        # ==========================================================
        logger.debug("   [Phase 1] Semantic Scholar API...")
        ...
        if s2_data:
            ...
            s2_summary = ", ".join(sources["s2"]) if sources["s2"] else "No new data"
            logger.debug(f"      -> {'OK' if sources['s2'] else 'MISS'} S2: {s2_summary}")
        else:
            logger.debug("      -> MISS: Not found in S2")

        # --- Phase 1 -> Web: Publisher Scraping (dari S2 links) ---
        if not keywords and (pdf_link or doi):
            logger.debug("   [Phase 1->Web] Scrape Publisher...")
            scrape_links = []
            ...
            for link_type, scrape_url in scrape_links:
                ...
                logger.debug(f"      -> {link_type}: {scrape_url[:50]}...")
            ...
            web_summary = ", ".join(sources["web"]) if sources["web"] else "No new data"
            logger.debug(f"      -> {'OK' if sources['web'] else 'MISS'} Web: {web_summary}")

        # PHASE 2: OPENALEX API (FREE)
        # ==========================================================
        oa_keywords_fallback = ""
        logger.debug("   [Phase 2] OpenAlex API...")
        ...
        if oa_data:
            ...
            if not keywords:
                ...
                if oa_scrape_links:
                    ...
                    for ltype, lurl in oa_scrape_links:
                        ...
                        seen.add(lurl)
                        logger.debug(f"      -> OA-Web {ltype}: {lurl[:40]}...")
            ...
            oa_summary = ", ".join(sources["oa"]) if sources["oa"] else "No new data"
            logger.debug(f"      -> {'OK' if sources['oa'] else 'MISS'} OA: {oa_summary}")
        else:
            logger.debug("      -> MISS: Not found in OpenAlex")

        # --- Fallback: OpenAlex AI Concepts as Keywords ---
        if not keywords and oa_keywords_fallback:
            keywords = oa_keywords_fallback
            logger.debug("   [Fallback] Using OpenAlex Concepts (AI) as Keywords")
        ...
        if not abstract or not keywords or not doi:
            ...
            logger.debug(f"   [Phase 3] BrightData Scholar (missing: {', '.join(missing)})...")
            ...
            if sh_data:
                ...
                if sch_links and (not keywords or not abstract):
                    ...
                    for ltype, lurl, use_proxy in sch_links:
                        ...
                        seen_urls.add(lurl)
                        logger.debug(f"      -> Scholar-Web {ltype}: {lurl[:30]}...")
                ...
                bd_summary = ", ".join(sources["bd"]) if sources["bd"] else "No new data"
                logger.debug(f"      -> {'OK' if sources['bd'] else 'MISS'} BD: {bd_summary}")
            else:
                logger.debug("      -> MISS: Not found in Scholar Search")
        else:
            stats["bd_skip"] += 1
            logger.debug("   [Phase 3] BrightData SKIPPED (data already complete)")
        ...
        logger.info(f"   RESULT: Abstract={'Y' if abstract else 'N'} Keywords={'Y' if keywords else 'N'} DOI={'Y' if doi else 'N'}")
        if all_sources:
            logger.debug(f"   Sources: {' -> '.join(all_sources)}")
        ...
        if avg_per_paper > 0:
            ...
            logger.info(f"Progress: {processed_count}/{test_limit or remaining} | ETA: {eta_min:.0f}m")
        else:
            logger.info(f"Progress: {processed_count}/{test_limit or remaining}")
    ...
    logger.info(f"Total papers processed: {processed_count} ({elapsed_total:.1f} min)")
    logger.info("COVERAGE RATE:")
    logger.info(f"   Author IDs : {stats['auth']}/{n} ({stats['auth']/n:.0%})")
    logger.info(f"   Keywords   : {stats['kw']}/{n} ({stats['kw']/n:.0%})")
    logger.info(f"   Abstract   : {stats['abs']}/{n} ({stats['abs']/n:.0%})")
    logger.info(f"   DOI        : {stats['doi']}/{n} ({stats['doi']/n:.0%})")
    logger.info(f"   TLDR       : {stats['tldr']}/{n} ({stats['tldr']/n:.0%})")
    logger.info("API HIT RATE:")
    logger.info(f"   Semantic Scholar : {stats['s2_hit']}/{n} ({stats['s2_hit']/n:.0%})")
    logger.info(f"   OpenAlex         : {stats['oa_hit']}/{n} ({stats['oa_hit']/n:.0%})")
    logger.info(f"   BrightData ($$)  : {stats['bd_hit']} calls | {stats['bd_skip']} skipped")
