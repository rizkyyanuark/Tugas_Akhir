...
class LightRAGGraphAdapter(GraphAdapter):
    ...
    def __init__(self, config: dict[str, Any] = None):
        ...
        # Use public Neo4j adapter instead of GraphDatabase
    ...
    def _get_metadata(self) -> GraphMetadata:
        ...
        return GraphMetadata(
            graph_type="lightrag",
            id_field="element_id",
            name_field="entity_id",  # LightRAG uses entity_id to store entity name
    ...
    async def query_nodes(self, keyword: str, **kwargs) -> dict[str, Any]:
        ...
        # If keyword is *, force max_depth=1 at least
        if keyword == "*":
    ...
    async def get_stats(self, **kwargs) -> dict[str, Any]:
        ...
        if not kb_id:
            # If no kb_id, may return global stats or empty
            return {"total_nodes": 0, "total_edges": 0, "entity_types": []}
    ...
    def normalize_node(self, raw_node: Any) -> dict[str, Any]:
        ...
        # Prioritize entity_id field as name (LightRAG style), if not present use name field
        # Often entity_id stores the entity name
    ...
    def _build_cypher_query(self, keyword: str, kb_id: str = None, limit: int = 50, max_depth: int = 0) -> str:
        """Build Cypher query"""
        # Security check: kb_id can only contain letters, numbers and underscores
        if kb_id:
        ...
        if kb_id:
            ...
            # This way nodes can be matched even without Entity label
            match_clause = f"MATCH (n:`{kb_id}`)"
        else:
            match_clause = "MATCH (n:Entity)"

        if keyword and keyword != "*":
            # Compatible with LightRAG format (entity_id) and normal format (name)
        ...
        # If max_depth > 0, we need to extend the query
        # To avoid over-complex queries, we use a two-step method:
        # 1. Find seed nodes
        # 2. Find relations around these nodes

        # Step 1: Find seed nodes
        # If keyword is * and there is kb_id, we use sampling logic for random sampling
        # But here query_nodes is mainly for search

        if max_depth > 0:
            # If extension is needed, return subgraph
            query = f"""
            {match_clause}
            {where_str}
            WITH n LIMIT {limit}
            ...
            # Simplified expansion query: only return seed nodes and their direct edges (if other end also in seeds, or no limit)
            # The query below returns seed nodes and relations between them, plus 1 hop neighbors
    ...
    def _process_query_result(self, result, limit: int = None) -> dict[str, list]:
        """Process query result and limit the number of nodes to limit"""
        nodes = []
        edges = []
        node_ids = set()
        edge_ids = set()
