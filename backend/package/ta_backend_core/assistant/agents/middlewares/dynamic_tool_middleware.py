...
from ta_backend_core.assistant.services.mcp_service import get_mcp_tools
from ta_backend_core.assistant.utils import logger


class DynamicToolMiddleware(AgentMiddleware):
    """Dynamic tool selection middleware — supports dynamic loading and registration of MCP tools.

    Note: All MCP tools that may be used must be preloaded and registered to `self.tools`
    during initialization. At runtime the middleware only filters tools based on configuration
    and cannot add new tools dynamically.
    """
    ...
    def __init__(self, base_tools: list[Any], mcp_servers: list[str] | None = None):
        ...
        Args:
            base_tools: list of base tools
            mcp_servers: optional list of MCP servers to preload
        """
        super().__init__()
    ...
    async def awrap_model_call(
        ...
        # Update the tools list in the request
