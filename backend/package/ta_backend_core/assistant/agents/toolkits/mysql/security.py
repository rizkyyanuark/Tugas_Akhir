...
class MySQLSecurityChecker:
    ...
    def validate_sql(cls, sql: str) -> bool:
        ...
        # Check whether it is an allowed operation
        if not any(sql_upper.startswith(op) for op in cls.ALLOWED_OPERATIONS):
            return False

        # Check dangerous keywords - only inspect the keyword at the beginning of the statement to avoid false positives
        # Extract the first word at the beginning of the statement
        first_word_match = re.match(r"^\s*(\w+)", sql_upper)
        first_word = first_word_match.group(1) if first_word_match else ""

        # Only check dangerous keywords at the beginning
    ...
    def validate_table_name(cls, table_name: str) -> bool:
        ...
        # Ensure the table name contains only letters, numbers, and underscores
