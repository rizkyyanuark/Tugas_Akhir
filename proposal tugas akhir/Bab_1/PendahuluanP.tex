...
Berdasarkan permasalahan tersebut, penelitian ini mengusulkan pengembangan sistem asisten pencarian literatur akademik yang mengimplementasikan \textit{Knowledge Graph-Based RAG} dengan mekanisme \textit{Hybrid Retrieval}. Penelitian ini tidak hanya bertujuan meningkatkan metrik akurasi pencarian semata, tetapi juga sejalan dengan tren kecerdasan buatan yang memanfaatkan \textit{Large Language Model} untuk mentransformasikan cara sivitas akademika berinteraksi dengan repositori pengetahuan serta mengubah proses tinjauan literatur yang manual menjadi dialog penemuan pengetahuan yang berbasis bukti (\textit{grounded inquiry}).

\section{Identifikasi Masalah}

\begin{enumerate}[label=\arabic*., leftmargin=0.75cm]
    \item Pertumbuhan literatur dan rekam jejak publikasi dosen pada domain Informatika dan Komputer di UNESA meningkat serta tersebar pada berbagai sumber, penelusuran literatur yang komprehensif dan efisien menjadi sulit.

    \item Sistem pencarian akademik konvensional dominan pada pencocokan kata kunci atau kemiripan teks, kurang mampu memodelkan konteks semantik dan relasi antar entitas akademik yang dibutuhkan untuk penelusuran literatur.

    \item Pemanfaatan \textit{Large Language Model} secara mandiri berisiko menghasilkan halusinasi dan jawaban yang tidak dapat ditelusuri sumbernya, kurang aman untuk kebutuhan akademik yang menuntut akurasi dan keterlacakan.

    \item Implementasi \textit{Retrieval-Augmented Generation} (\textit{RAG}) berbasis \textit{vector similarity} cenderung mengambil potongan teks semata, kurang optimal untuk pertanyaan yang membutuhkan pemahaman relasional dan pencocokan konteks lintas entitas seperti pencarian literatur.

    \item Belum tersedia sistem penelusuran literatur pada lingkup Infokom UNESA yang mengimplementasikan \textit{knowledge graph} untuk mendukung generasi jawaban model bahasa besar.
\end{enumerate}

\section{Rumusan Masalah}

\begin{enumerate}[label=\arabic*., leftmargin=0.75cm]
    \item Bagaimana merancang \textit{pipeline} konstruksi \textit{Knowledge Graph} yang mampu mengekstraksi entitas dan relasi dari korpus literatur akademik untuk mendukung representasi pengetahuan yang komprehensif?

    \item Bagaimana merancang mekanisme \textit{Hybrid Vector-Graph Retrieval} dalam arsitektur \textit{Knowledge Graph Based RAG} pada sistem \textit{Question Answering} secara sinergis?

    \item Sejauh mana integrasi \textit{Knowledge Graph} dapat mengurangi halusinasi pada model bahasa besar (\textit{LLM}) dan meningkatkan transparansi (\textit{traceability}) referensi dalam jawaban yang dihasilkan?
\end{enumerate}

\section{Tujuan Penelitian}

\begin{enumerate}[label=\arabic*., leftmargin=0.75cm]
    \item Merancang \textit{pipeline} konstruksi \textit{Knowledge Graph} yang mampu mengekstraksi dan menstandarisasi entitas serta relasi dari korpus literatur akademik secara otomatis.

    \item Mengembangkan mekanisme \textit{Hybrid Vector-Graph Retrieval} pada arsitektur \textit{Knowledge Graph Based RAG} untuk meningkatkan relevansi dan kontekstualitas pencarian.

    \item Menganalisis efektivitas integrasi \textit{Knowledge Graph} dalam meminimalkan tingkat halusinasi model bahasa besar (\textit{LLM}) serta meningkatkan kemampuan pelacakan (\textit{traceability}) sumber referensi.
\end{enumerate}
...
\begin{enumerate}[label=\arabic*., leftmargin=0.75cm]
    ...
    \item \textit{Input} data utama dibatasi pada metadata publikasi (judul, penulis, tahun, afiliasi) dan teks abstrak, tanpa melakukan pengunduhan isi full dokumen PDF.
