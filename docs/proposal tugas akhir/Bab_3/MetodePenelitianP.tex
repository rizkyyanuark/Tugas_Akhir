...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=10pt]
    ...
    \item \textbf{Persiapan Data (\textit{Data Preparation})}
          ...
          \begin{enumerate}[label=\textbf{\alph*.}, leftmargin=0.75cm, itemsep=4pt]
              ...
              \item \textbf{\textit{Embedding Generation}:} Setiap \textit{chunk}, entitas, relasi, dan himpunan kata kunci dikonversi menjadi vektor 1.024 dimensi menggunakan model \texttt{Qwen/Qwen3-Embedding-0.6B} melalui layanan SiliconFlow. Representasi tersebut memungkinkan pencarian berbasis makna (\textit{semantic search}) yang melampaui pencocokan kata kunci leksikal.
          \end{enumerate}
    ...
    \item \textbf{Konstruksi \textit{Knowledge Graph}}
          ...
          \begin{figure}[htbp]
              ...
              \begin{tikzpicture}[scale=1, transform shape]
                  ...
                  \node (re) [process, minimum width=1.9cm, minimum height=0.7cm, text width=1.1cm] at (6.5, -8.1) {Ontology\\Mapping};
          ...
          \begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=0.75cm, itemsep=5pt]
              \item \textbf{\textit{Ontology Modeling (Pemodelan Skema)}}
                    ...
                    \hspace*{0.75cm}Skema ontologi dirancang sebagai \textit{blueprint} \textit{Knowledge Graph} sebelum proses ekstraksi dimulai (\ref{fig:ontology-design}). Relasi aktif dikelompokkan menjadi: (1) relasi metadata akademik, seperti \textit{publishes}, \textit{has\_author}, \textit{has\_affiliation}, \textit{published\_in\_year}, \textit{published\_in\_venue}, dan \textit{collaborates\_with}; (2) relasi semantik publikasi, seperti \textit{has\_topic}, \textit{uses\_method}, \textit{uses\_model}, \textit{evaluated\_with}, \textit{has\_result}, dan \textit{belongs\_to\_domain}; serta (3) relasi SKOS untuk menghubungkan konsep IEEE. Relasi \textit{cites} disediakan dalam rancangan, tetapi hanya diaktifkan apabila data referensi atau DOI yang disitasi tersedia. Pembatasan ini mencegah pembentukan relasi yang tidak didukung data sumber.
              ...
              \item \textbf{\textit{Parallel Processing Pipeline}}
                    ...
                    \begin{enumerate}[label=\textbf{\alph*.}, leftmargin=0.75cm, itemsep=5pt]
                        ...
                        \item \textbf{Jalur Data Tidak Terstruktur: \textit{NLP-Based Extraction}}
                              ...
                              \hspace*{0.75cm}Tahap awal \textbf{\textit{Text Processing}} membersihkan abstrak, TLDR, dan kata kunci dari karakter yang tidak relevan, lalu menyatukannya sebagai teks akademik untuk proses ekstraksi konsep.

                              \hspace*{0.75cm}Ekstraksi entitas tidak terstruktur diarahkan menggunakan \textit{GLiNER} \cite{zaratiana2023gliner} sebagai jalur opsional \textit{zero-shot Named Entity Recognition}. Entitas yang ditemukan tidak langsung dianggap sebagai relasi bebas. Setiap entitas terlebih dahulu dipetakan ke skema ontologi penelitian, seperti \textit{Method}, \textit{Model}, \textit{Dataset}, \textit{Metric}, \textit{Domain}, atau \textit{ResearchTopic}. Setelah tipe entitas jelas, relasi dibentuk secara deterministik mengikuti ontologi, misalnya \texttt{USES\_MODEL}, \texttt{EVALUATED\_WITH}, \texttt{USES\_METHOD}, \texttt{HAS\_TOPIC}, dan \texttt{BELONGS\_TO\_DOMAIN}. Dengan cara ini, graf tetap terkontrol dan tidak bergantung pada tebakan relasi bebas dari model. \textit{GLiREL} diposisikan sebagai opsi eksperimen lanjutan, bukan jalur utama pada pembangunan graf yang dilaporkan.
              ...
              \item \textbf{\textit{Dual Indexing}}
                    ...
                    \begin{enumerate}[label=\textbf{\alph*.}, leftmargin=0.75cm, itemsep=5pt]
                        \item \textbf{\textit{Vector Indexing}}
                              ...
                              \hspace*{0.75cm}Representasi tekstual publikasi dipotong menjadi segmen maksimum 2.024 karakter dengan \textit{overlap} 50 karakter. Setiap potongan teks, entitas, relasi, dan petunjuk kata kunci dikonversi menjadi vektor 1.024 dimensi menggunakan \texttt{Qwen/Qwen3-Embedding-0.6B} melalui SiliconFlow. Vektor disimpan pada Zilliz Cloud yang kompatibel dengan Milvus dalam koleksi \texttt{PaperChunk}, \texttt{EntityEmbedding}, \texttt{RelationshipEmbedding}, dan \texttt{ContentKeyword}.
    ...
    \item \textbf{Evaluasi Sistem}
          ...
          \hspace*{0.75cm}Penelitian ini mengevaluasi kinerja sistem menggunakan pendekatan multi-lapis untuk mengukur kualitas pencarian dokumen referensi (\textit{retrieval quality}) dan kualitas sintesis jawaban (\textit{generation quality}). Kerangka kerja evaluasi terdiri dari 94 pertanyaan uji, yang terbagi menjadi 90 \textit{ranked cases} (pertanyaan dengan jawaban acuan/\textit{ground truth}) dan 4 \textit{guardrail cases} (pertanyaan di luar cakupan untuk menguji penolakan sistem). Kumpulan pertanyaan tersebut dikelompokkan ke dalam tiga kategori utama: Kategori A (faktual, $n=30$), Kategori B (relasional, $n=30$), dan Kategori C (\textit{multi-hop}/sintesis, $n=30$).

          \hspace*{0.75cm}Evaluasi dilakukan melalui tiga lapisan pengukuran yang saling melengkapi sebagai berikut:

          \begin{enumerate}[label=\arabic*., leftmargin=0.75cm, itemsep=4pt]

              \item \textbf{Evaluasi Akurasi \textit{Retrieval} (Layer 1)}:
                    Mengukur kemampuan pencarian dalam menemukan dokumen ilmiah yang relevan menggunakan metrik \textit{Mean Reciprocal Rank} (MRR) dan \textit{Hit@K} (untuk $K = 1, 5, 10$) \citep{manning2008introduction}.

                    \textit{Mean Reciprocal Rank} (MRR) didefinisikan sebagai rata-rata kebalikan dari posisi peringkat dokumen relevan pertama yang berhasil ditemukan:
                    \begin{equation}
                        \textit{MRR} = \frac{1}{|Q|} \sum_{i=1}^{|Q|} \frac{1}{\text{rank}_i}
                    \end{equation}
                    di mana $|Q|$ melambangkan total kueri dan $\text{rank}_i$ merupakan peringkat posisi dokumen relevan pertama pada kueri ke-$i$.

                    \textit{Hit@K} mengukur proporsi kueri di mana setidaknya satu dokumen relevan berhasil diambil dalam peringkat $K$ teratas:
                    \begin{equation}
                        \textit{Hit@K} = \frac{\text{Jumlah kueri dengan dokumen relevan di top } K}{|Q|}
                    \end{equation}

              \item \textbf{Evaluasi Kualitas Jawaban Berbasis RAGAS (Layer 2)}:
                    Mengevaluasi aspek kualitas pembuatan teks secara otomatis menggunakan kerangka kerja RAGAS \citep{es-etal-2024-ragas}. Metrik yang digunakan meliputi \textit{Faithfulness} untuk mengukur tingkat kebebasan dari halusinasi dan \textit{Answer Relevancy} untuk mengukur keselarasan respons terhadap pertanyaan.

                    \textit{Faithfulness} didefinisikan sebagai rasio klaim atau pernyataan dalam jawaban ($V$) yang dapat diverifikasi secara langsung terhadap dokumen konteks pendukung ($S$):
                    \begin{equation}
                        \textit{Faithfulness} = \frac{|V|}{|S|}
                    \end{equation}

                    \textit{Answer Relevancy} mengukur relevansi semantik jawaban yang dihasilkan terhadap kueri pengguna dengan menghitung rata-rata kemiripan kosinus (\textit{cosine similarity}) dari representasi vektor kueri asli terhadap beberapa pertanyaan buatan yang dibangkitkan dari jawaban tersebut:
                    \begin{equation}
                        \textit{Answer Relevancy} = \frac{1}{m} \sum_{i=1}^{m} \cos(E_{q_i}, E_q)
                    \end{equation}
                    di mana $E_q$ adalah representasi vektor dari kueri asli, $E_{q_i}$ adalah vektor kueri buatan ke-$i$, dan $m$ adalah jumlah kueri buatan.

              \item \textbf{Evaluasi Hakim LLM Berpasangan (\textit{Pairwise LLM Judge} - Layer 4)}:
                    Menggunakan model bahasa besar (\textit{DeepSeek v4-flash}) sebagai hakim otomatis (\textit{LLM-as-a-Judge}) untuk membandingkan performa jawaban sistem yang diusulkan (\textit{Subgraph} dan \textit{Hybrid}) terhadap baseline (\textit{Naive}) secara berpasangan (\textit{pairwise}) \citep{zheng2023judging}. Evaluasi ini menilai respons berdasarkan empat dimensi utama: \textit{Faithfulness}, \textit{Traceability} (kemudahan penelusuran bukti referensi), \textit{Comprehensiveness} (kelengkapan informasi), serta penilaian keseluruhan (\textit{Overall}). Hasil evaluasi dinyatakan dalam bentuk \textit{Win Rate} (persentase kemenangan terhadap baseline):
                    \begin{equation}
                        \textit{Win Rate} = \frac{\text{Jumlah kueri dimenangkan oleh mode } M}{|Q|} \times 100\%
                    \end{equation}

          \end{enumerate}

          \hspace*{0.75cm}Melalui skenario evaluasi multi-lapis ini, kelebihan dan kelemahan masing-masing mode pencarian (\textit{Naive}, \textit{Subgraph}, dan \textit{Hybrid}) dapat diidentifikasi secara komprehensif tanpa bergantung pada ambang batas keberhasilan tunggal. Hasil evaluasi disimpan secara terstruktur dan diekspor dalam bentuk tabel serta diagram radar yang disajikan pada Bab IV.


          \hspace*{0.75cm}Mode \texttt{naive} diperlakukan sebagai representasi \textit{Vector RAG} karena hanya mengandalkan pencarian semantik pada \texttt{PaperChunk}. Mode \texttt{subgraph} memanfaatkan simpul dan jalur graf lokal. Mode \texttt{global} menggunakan relasi yang diretrieval dari indeks \texttt{RelationshipEmbedding}. Mode \texttt{hybrid} menggabungkan konteks lokal dan global, sedangkan mode \texttt{mix} menggabungkan bukti tekstual, petunjuk kata kunci, entitas, relasi, dan subgraf. Hasil evaluasi disimpan dalam \texttt{notebooks/build-graph/outputs/evaluation} dan diekspor menjadi tabel serta gambar yang dapat langsung digunakan pada Bab IV.


          % 6. GUI Implementation
    \item \textbf{Implementasi GUI (\textit{GUI Implementation})}

          \hspace*{0.75cm}Antarmuka pengguna dikembangkan sebagai aplikasi web yang mengadaptasi backbone Yuxi. Pada sisi pengguna, sistem disajikan sebagai antarmuka percakapan sehingga dosen atau pengguna lain dapat mengajukan pertanyaan dalam bahasa natural. Pada sisi belakang, pertanyaan tersebut diteruskan ke \textit{agent} dan alat \texttt{query\_kb} untuk menjalankan proses \textit{Academic GraphRAG} terhadap Neo4j dan Zilliz/Milvus.

          \hspace*{0.75cm}GUI tidak digunakan untuk membangun graf, tetapi menjadi lapisan interaksi dan inspeksi hasil. Antarmuka ini menampilkan jawaban akhir, sumber publikasi, potongan dokumen, \textit{graph triples}, serta panel bukti akademik yang membantu pengguna melihat dari mana jawaban diperoleh. Dengan demikian, GUI diposisikan sebagai penghubung antara pengguna dan sistem \textit{Hybrid GraphRAG}, bukan sebagai pengganti pipeline konstruksi KG.
\end{enumerate}
