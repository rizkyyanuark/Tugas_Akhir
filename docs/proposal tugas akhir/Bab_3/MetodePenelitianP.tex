...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=10pt]
    ...
    \item \textbf{Evaluasi Sistem}
          ...
          \hspace*{0.75cm}Evaluasi sistem disusun untuk memisahkan kualitas pengambilan konteks, kualitas jawaban, dan keterlacakan bukti. Pemisahan ini diperlukan karena sistem \textit{Hybrid GraphRAG} tidak hanya menghasilkan jawaban akhir, tetapi juga mengambil \textit{paper chunks}, entitas, relasi, dan subgraf dari dua basis data yang berbeda.

          \hspace*{0.75cm}Dataset evaluasi terdiri atas 60 pertanyaan, yaitu 56 \textit{ranked cases} dan 4 \textit{guardrail cases}. Pertanyaan utama dibagi menjadi kategori faktual, relasional, dan \textit{multi-hop}. Kategori faktual digunakan untuk menguji kemampuan menemukan publikasi atau atribut tertentu. Kategori relasional digunakan untuk menguji hubungan dosen, publikasi, topik, metode, dan kolaborasi. Kategori \textit{multi-hop} digunakan untuk menguji pertanyaan yang memerlukan penggabungan beberapa hubungan dalam graf. Sementara itu, \textit{guardrail cases} digunakan untuk memeriksa apakah sistem mampu menolak atau membatasi jawaban ketika informasi tidak tersedia dalam basis pengetahuan.

          \hspace*{0.75cm}Evaluasi dilakukan dalam empat lapis. Lapis pertama mengukur kualitas \textit{retrieval} menggunakan \textit{Hit@K}, \textit{Mean Reciprocal Rank} (MRR), \textit{Precision@K}, dan latensi. Lapis kedua mengukur kualitas jawaban menggunakan metrik berbasis RAGAS dan heuristik lokal, seperti \textit{faithfulness}, \textit{answer relevancy}, \textit{context precision}, dan \textit{context recall}. Lapis ketiga membandingkan performa beberapa mode retrieval, yaitu \texttt{naive}, \texttt{subgraph}, \texttt{hybrid}, dan \texttt{mix}. Lapis keempat menggunakan \textit{LLM-as-a-judge} untuk membandingkan jawaban \textit{Vector RAG} sebagai baseline terhadap mode berbasis graf berdasarkan dimensi \textit{faithfulness}, \textit{traceability}, \textit{comprehensiveness}, dan penilaian keseluruhan.
