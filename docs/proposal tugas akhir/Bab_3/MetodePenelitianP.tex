...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=10pt]
    ...
    \item \textbf{Evaluasi Sistem}
          ...
          \hspace*{0.75cm}Penelitian ini mengevaluasi kinerja sistem menggunakan pendekatan multi-lapis untuk mengukur kualitas pencarian dokumen referensi (\textit{retrieval quality}) dan kualitas sintesis jawaban (\textit{generation quality}). Kerangka kerja evaluasi terdiri dari 60 pertanyaan uji, yang terbagi menjadi 56 \textit{ranked cases} (pertanyaan dengan jawaban acuan/\textit{ground truth}) dan 4 \textit{guardrail cases} (pertanyaan di luar cakupan untuk menguji penolakan sistem). Kumpulan pertanyaan tersebut dikelompokkan ke dalam tiga kategori utama: Kategori A (faktual, $n=20$), Kategori B (relasional, $n=30$), dan Kategori C (\textit{multi-hop}/sintesis, $n=6$).
